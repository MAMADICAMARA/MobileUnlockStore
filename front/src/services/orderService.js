...
const orderService = {
  ...
  getMyOrders: (params = {}) => {
    ...
    const qs = q.toString();
    return api.get(`/api/orders/my${qs ? `?${qs}` : ''}`);
  },

  // Alias pour rétrocompatibilité
  getOrders:  ()         => orderService.getMyOrders(),
  getHistory: (p = {})   => orderService.getMyOrders(p),
  ...
  // ✅ FIX M-14 : route réelle dans orderRoutes.js
  // AVANT : GET /api/admin/orders/:id  ← n'existe pas
  // APRÈS : GET /api/orders/admin/:id  ← route réelle
  getAdminOrderById: (orderId) => api.get(`/api/orders/admin/${orderId}`),

  // ✅ FIX M-14 : route réelle dans adminRoutes.js
  // AVANT : PUT /api/admin/orders/:id  ← n'existe pas
  // APRÈS : PUT /api/admin/orders/:id/status ← route réelle
  updateOrderStatus: (orderId, status, extra = {}) =>
    api.put(`/api/admin/orders/${orderId}/status`, { status, ...extra }),

  // Mise à jour complète (statut + livraison + notes) via orderRoutes
...
export const formatOrderStatus = (status) => {
  const map = {
    ...
    'Remboursé':  { label: '💰 Remboursé',   color: 'text-purple-700', bgColor: 'bg-purple-100', borderColor: 'border-purple-300' },
  };
  return map[status] || { label: status || '—', color: 'text-gray-700', bgColor: 'bg-gray-100', borderColor: 'border-gray-300' };
};

// Une commande confirmée ne peut plus être annulée par le client
export const isOrderCancellable = () => false;

export const canUploadDocuments = (order) => order?.status === 'En cours';
...
// export const formatOrderStatus = (status) => {
//   const statusMap = {
//     'En cours': { label: '🔄 En cours', color: 'text-blue-700',  bgColor: 'bg-blue-100',  borderColor: 'border-blue-300' },
//     'Annulé':   { label: '❌ Annulé',   color: 'text-red-700',   bgColor: 'bg-red-100',   borderColor: 'border-red-300' },
//     'Terminé':  { label: '✅ Terminé',  color: 'text-green-700', bgColor: 'bg-green-100', borderColor: 'border-green-300' },
//   };
//   return statusMap[status] || { label: status || '—', color: 'text-gray-700', bgColor: 'bg-gray-100', borderColor: 'border-gray-300' };
// };
...
//  */
// export const isOrderCancellable = (_order) => false;
