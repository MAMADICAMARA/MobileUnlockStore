...
const adminService = {
  ...
  // ✅ Détail commande avec infos client complètes (via orderRoutes /admin/:id)
  getOrderById: (orderId) => api.get(`/api/orders/admin/${orderId}`),

  // ✅ Mise à jour statut — PUT + body { status } (correspond au backend adminRoutes)
  updateOrderStatus: (orderId, newStatus) =>
    api.put(`/api/admin/orders/${orderId}/status`, { status: newStatus }),

  // ✅ Mise à jour complète commande (statut + livraison + notes) via orderRoutes
