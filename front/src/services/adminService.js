...
const adminService = {
  ...
  getProviderById:        (id)     => api.get(`/api/providers/${id}`),
  ...
  testProviderConnection: (id)     => api.post(`/api/providers/${id}/test`),

  // ─── ASSOCIATIONS SERVICE ↔ FOURNISSEUR ────────────────────────────────────
  getAllProviderServices: ()         => api.get('/api/provider-services'),
  createProviderService:  (data)     => api.post('/api/provider-services', data),
  updateProviderService:  (id, data) => api.put(`/api/provider-services/${id}`, data),
  deleteProviderService:  (id)       => api.delete(`/api/provider-services/${id}`),

  // ─── TABLEAU DE BORD FOURNISSEURS ───────────────────────────────────────────
  getProviderOrders: (params = {}) => {
    const q = new URLSearchParams(params).toString();
    return api.get(`/api/provider-orders${q ? `?${q}` : ''}`);
  },
  retryProviderOrder:  (id) => api.post(`/api/provider-orders/${id}/retry`),
  refundProviderOrder: (id) => api.post(`/api/provider-orders/${id}/refund`),

  // ─── PAIEMENTS AUTOMATIQUES — administration ────────────────────────────────
  getPaymentSettings:        ()       => api.get('/api/payments/settings'),
  updatePaymentSettings:     (data)   => api.put('/api/payments/settings', data),
  getPaymentProviders:       ()       => api.get('/api/payments/providers'),
  getPaymentProviderById:    (id)     => api.get(`/api/payments/providers/${id}`),
  updatePaymentProvider:     (id, data) => api.put(`/api/payments/providers/${id}`, data),
  togglePaymentProvider:     (id)     => api.patch(`/api/payments/providers/${id}/toggle`),
  testPaymentProvider:       (id)     => api.post(`/api/payments/providers/${id}/test`),
  getAllPaymentTransactions: (params = {}) => {
    const q = new URLSearchParams(params).toString();
    return api.get(`/api/payments/transactions${q ? `?${q}` : ''}`);
  },
  getPaymentTransactionById: (id) => api.get(`/api/payments/transactions/${id}`),
};
