...
const OrderHistoryPage = () => {
  ...
  // Ref pour accéder à selectedOrder dans le polling sans le mettre dans les deps
  const selectedOrderRef = useRef(null);
  useEffect(() => { selectedOrderRef.current = selectedOrder; }, [selectedOrder]);

  const categoryTabs = [
    { key: 'Tous',     label: '📋 Toutes mes commandes' },
  ...
  const normalizeStatus = (status) => {
    ...
    if (s === 'terminé'  || s === 'completed')  return 'Terminé';
    if (s === 'annulé'   || s === 'cancelled' || s === 'remboursé') return 'Annulé';
    return 'En cours'; // 'en cours', 'pending', 'processing', ou toute autre valeur
  };

  const statusConfig = {
    'En cours': { icon: Clock,       labelFr: '🔄 En cours', badgeColor: 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300',   borderColor: 'border-amber-200 dark:border-amber-800'  },
    'Terminé':  { icon: CheckCircle, labelFr: '✅ Terminé',  badgeColor: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',   borderColor: 'border-green-200 dark:border-green-800'  },
    'Annulé':   { icon: X,           labelFr: '❌ Annulé',   badgeColor: 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300',           borderColor: 'border-red-200 dark:border-red-800'      },
  };
  ...
  const fetchOrders = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const normalized = arr.map(o => ({ ...o, status: normalizeStatus(o.status) }));
      setOrders(normalized);
      setLastUpdated(new Date());

      const total      = normalized.length;
      const completed  = normalized.filter(o => o.status === 'Terminé').length;
      const pending    = normalized.filter(o => o.status === 'En cours').length;
      const totalSpent = normalized.filter(o => o.status === 'Terminé').reduce((s, o) => s + (o.amount || 0), 0);
      setStats({ total, completed, pending, totalSpent });

      if (!silent) {
        const params = new URLSearchParams(location.search);
        setFilterCategory(params.get('category') || 'Tous');
      }

      // Mise à jour du modal si ouvert — via ref pour éviter la boucle de deps
      const current = selectedOrderRef.current;
      if (silent && current) {
        const updated = normalized.find(o => o._id === current._id);
  ...
  }, [location.search]); // selectedOrder retiré des deps — géré via ref

  // Chargement initial
  useEffect(() => { fetchOrders(); }, [fetchOrders]);

  // ✅ Polling toutes les 15 secondes (plus réactif que 30s)
  useEffect(() => {
  ...
  // ── Utilitaires ───────────────────────────────────────────────────────────
  const formatDate  = (d) => { if (!d) return '—'; return new Intl.DateTimeFormat('fr-FR', { day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit' }).format(new Date(d)); };
  const formatPrice = (p) => { if (!p && p !== 0) return '—'; return `${new Intl.NumberFormat('fr-FR').format(p)} FG`; };
  const handleSort  = (key) => setSortConfig(prev => ({ key, direction: prev.key === key && prev.direction === 'asc' ? 'desc' : 'asc' }));

  const handleCategoryChange = (catKey) => {
    setFilterCategory(catKey);
    const params = new URLSearchParams(location.search);
    if (catKey === 'Tous') params.delete('category'); else params.set('category', catKey);
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (filterCategory !== 'Tous') {
      list = list.filter(o => {
        const cat = (o?.serviceDetails?.category || o?.service?.category || '').toString().toLowerCase().trim();
        return cat === filterCategory.toLowerCase().trim();
      });
    }
    ...
    if (searchTerm) {
      ...
      list = list.filter(o =>
        (o?.serviceDetails?.name || o?.service?.name || '').toLowerCase().includes(q) ||
    ...
    list.sort((a, b) => {
      let av, bv;
      if (sortConfig.key === 'serviceName') { av = a?.serviceDetails?.name || ''; bv = b?.serviceDetails?.name || ''; }
      else if (sortConfig.key === 'createdAt') { av = new Date(a?.createdAt || 0); bv = new Date(b?.createdAt || 0); }
      else if (sortConfig.key === 'amount')    { av = a?.amount || 0;              bv = b?.amount || 0;              }
      else return 0;
  ...
  const openOrderDetails  = (order) => { setSelectedOrder(order); setShowDetailsModal(true); };
  const closeOrderDetails = () => { setShowDetailsModal(false); setSelectedOrder(null); };
  ...
  const StatCard = ({ title, value, icon: Icon, color }) => (
    <div className="bg-white dark:bg-slate-800 rounded-xl p-4 sm:p-6 shadow-lg border border-gray-100 dark:border-gray-700">
  ...
  return (
    <div className="space-y-4 sm:space-y-6 p-3 sm:p-6">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm flex items-center gap-2 flex-wrap">
            ...
            {lastUpdated && (
              <span className="text-xs text-gray-400">
                · Dernière sync : {lastUpdated.toLocaleTimeString('fr-FR', { hour:'2-digit', minute:'2-digit', second:'2-digit' })}
              </span>
            )}
          </p>
        </div>
        <button onClick={() => fetchOrders()}
          className="self-start sm:self-auto flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 transition-colors shadow-sm">
      ...
      {/* Onglets catégorie */}
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-3 sm:p-4 border border-gray-100 dark:border-gray-700">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-3 sm:p-4 border border-gray-100 dark:border-gray-700">
        <div className="flex flex-col gap-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Rechercher par service, IMEI, numéro de commande..."
              value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-10 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white text-sm" />
            <button onClick={() => setShowFilters(!showFilters)}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 text-gray-400 hover:text-gray-600 transition-colors">
              <Filter className={`w-5 h-5 transition-transform duration-200 ${showFilters ? 'rotate-180 text-blue-500' : ''}`} />
            </button>
          </div>
          ...
          {showFilters && (
            <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
              ...
              <div className="flex flex-wrap gap-2">
                {['Tous', 'En cours', 'Terminé', 'Annulé'].map(status => (
                  <button key={status} onClick={() => setFilterStatus(status)}
                    ...
                    {status === 'Tous' ? '✨ Tous les statuts' : statusConfig[status]?.labelFr || status}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              ...
              <span className="text-sm text-blue-600 dark:text-blue-400 font-medium">{filteredOrders.length} commande(s)</span>
            </div>
            {searchTerm && <button onClick={() => setSearchTerm('')} className="text-sm text-gray-500 hover:text-gray-700">Effacer</button>}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden border border-gray-100 dark:border-gray-700">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-500 dark:text-gray-400">Chargement de vos commandes...</p>
          </div>
        ) : error ? (
          <div className="text-center py-16 px-4">
            <AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4" />
            <p className="text-red-500 mb-4">{error}</p>
            <button onClick={() => fetchOrders()} className="px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition">Réessayer</button>
          </div>
          ...
          <div className="text-center py-16 px-4">
            ...
            <p className="text-gray-500 dark:text-gray-400 font-medium">Aucune commande trouvée</p>
            <p className="text-sm text-gray-400 mt-1">{searchTerm ? 'Modifiez vos critères' : 'Vos commandes apparaîtront ici'}</p>
          </div>
        ) : (
          <>
            {/* Tableau desktop */}
            <div className="hidden sm:block overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-700">
                  <tr>
                    {[['Service / Réf','serviceName'],['Date d\'achat','createdAt'],['Statut',null],['Prix total','amount'],['Détails',null]].map(([label, key]) => (
                      <th key={label} className={`px-6 py-4 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider ${label==='Prix total'?'text-right':label==='Détails'?'text-center':'text-left'}`}>
                        {key ? <button onClick={() => handleSort(key)} className="group flex items-center gap-1 hover:text-gray-700 transition-colors">{label}<SortIcon column={key} /></button> : label}
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredOrders.map((order) => {
                    const sName  = order?.serviceDetails?.name || order?.service?.name || 'Service générique';
                    const conf   = statusConfig[order.status] || statusConfig['pending'];
                    const SIcon  = conf.icon;
                    return (
                      <tr key={order._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer" onClick={() => openOrderDetails(order)}>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{sName}</p>
                              <p className="text-xs text-gray-400 font-mono">ID: #{order._id?.slice(-10)}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center text-sm text-gray-500 gap-2">
                            <Calendar className="w-4 h-4 flex-shrink-0" />{formatDate(order?.createdAt)}
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${conf.badgeColor} border ${conf.borderColor}`}>
                            <SIcon className="w-3.5 h-3.5" />{conf.labelFr}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-right">
                          <span className="font-bold text-gray-900 dark:text-white whitespace-nowrap">{formatPrice(order?.amount)}</span>
                        </td>
                        <td className="px-6 py-4 text-center">
                          <button onClick={(e) => { e.stopPropagation(); openOrderDetails(order); }}
            ...
            <div className="sm:hidden divide-y divide-gray-200 dark:divide-gray-700">
              {filteredOrders.map((order) => {
                ...
                const SIcon = conf.icon;
                return (
                  <div key={order._id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer" onClick={() => openOrderDetails(order)}>
                    <div className="flex justify-between items-start mb-2 gap-2">
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-gray-900 dark:text-white text-sm truncate">{sName}</p>
                        <p className="text-xs text-gray-400 font-mono">#{order._id?.slice(-8)}</p>
                      </div>
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold flex-shrink-0 ${conf.badgeColor} border ${conf.borderColor}`}>
                        <SIcon className="w-3 h-3" />{conf.labelFr}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      <div className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />{formatDate(order?.createdAt)}</div>
                      <span className="font-bold text-gray-900 dark:text-white text-sm">{formatPrice(order?.amount)}</span>
                    </div>
                  </div>
                );
              })}
      ...
      {showDetailsModal && selectedOrder && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto border border-gray-100 dark:border-gray-700">

            {/* Header modal */}
            <div className="sticky top-0 bg-white dark:bg-slate-800 p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center z-10">
              <div>
                <h2 className="text-lg sm:text-2xl font-bold text-gray-900 dark:text-white">Détails de la commande</h2>
                <p className="text-xs text-gray-500 font-mono mt-0.5">
                  ID : <span className="bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded">{selectedOrder._id}</span>
                </p>
              </div>
              <button onClick={closeOrderDetails} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 rounded-xl transition-colors">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
              ...
              {/* Statut + facturation */}
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/10 dark:to-purple-900/10 rounded-xl p-4 border border-blue-100 dark:border-blue-900/30">
                <h3 className="font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2 text-sm sm:text-base">
                  <Info className="w-5 h-5 text-blue-500" /> Statut général & Facturation
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                  <div>
                    <p className="text-xs text-gray-400 uppercase font-semibold mb-1">Statut actuel</p>
                    {(() => {
                      const conf = statusConfig[selectedOrder.status] || statusConfig['pending'];
                      const SIcon = conf.icon;
                      return (
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${conf.badgeColor} border ${conf.borderColor}`}>
                          <SIcon className="w-3.5 h-3.5" />{conf.labelFr}
                  ...
                  <div>
                    <p className="text-xs text-gray-400 uppercase font-semibold mb-1">Montant prélevé</p>
                    <p className="text-lg font-black text-emerald-600 dark:text-emerald-400">{formatPrice(selectedOrder.amount)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400 uppercase font-semibold mb-1">Date de soumission</p>
                    <p className="text-gray-800 dark:text-gray-200 font-medium text-sm">{formatDate(selectedOrder.createdAt)}</p>
                  </div>
                </div>
              </div>

              {/* Propriétés du service */}
              <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 space-y-4">
                <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2 text-sm sm:text-base">
                  <Layers className="w-5 h-5 text-purple-500" /> Propriétés du Service
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm border-b pb-4 dark:border-gray-700">
                  <div>
                    <p className="text-xs text-gray-400">Nom du produit / service</p>
                    <p className="font-semibold text-gray-900 dark:text-white mt-0.5">{selectedOrder?.serviceDetails?.name || selectedOrder?.service?.name || 'Non spécifié'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Catégorie technique</p>
                    <span className="inline-block mt-1 uppercase text-xs font-mono bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded">
                      {selectedOrder?.serviceDetails?.category || selectedOrder?.service?.category || 'Générique'}
                    </span>
                  </div>
                </div>

                {/* IMEI / SN / infos spécifiques */}
                <div className="space-y-3 pt-1">
                  {selectedOrder?.imei && (
                    <div className="bg-gray-50 dark:bg-gray-700/40 p-3 rounded-lg flex items-start gap-2.5">
                      <Hash className="w-4 h-4 mt-0.5 text-gray-400 flex-shrink-0" />
                      <div>
                        <p className="text-xs text-gray-400 font-medium">Code IMEI renseigné</p>
                        <p className="font-mono text-sm font-bold text-gray-900 dark:text-white tracking-wider">{selectedOrder.imei}</p>
                      </div>
                    </div>
                  )}
                  {selectedOrder?.serialNumber && (
                    <div className="bg-gray-50 dark:bg-gray-700/40 p-3 rounded-lg flex items-start gap-2.5">
                      <Hash className="w-4 h-4 mt-0.5 text-gray-400 flex-shrink-0" />
                      <div>
                        <p className="text-xs text-gray-400 font-medium">Numéro de Série (SN)</p>
                        <p className="font-mono text-sm font-bold text-gray-900 dark:text-white">{selectedOrder.serialNumber}</p>
                      </div>
                    </div>
                  )}

                  {/* Données userSubmittedData */}
                  {selectedOrder?.userSubmittedData && Object.keys(selectedOrder.userSubmittedData).length > 0 && (
                    <div className="space-y-2">
                      {Object.entries(selectedOrder.userSubmittedData).map(([key, value]) => (
                        <div key={key} className="bg-gray-50 dark:bg-gray-700/40 p-3 rounded-lg flex items-start gap-2.5">
                  ...
                  {selectedOrder?.additionalInfo && (
                    <div className="bg-gray-50 dark:bg-gray-700/40 p-3 rounded-lg flex items-start gap-2.5">
                      <FileText className="w-4 h-4 mt-0.5 text-gray-400 flex-shrink-0" />
                      <div>
                        <p className="text-xs text-gray-400 font-medium">Notes / Instructions</p>
                        <p className="text-sm text-gray-800 dark:text-gray-200">{selectedOrder.additionalInfo}</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {/* Livraison / réponse admin */}
              {(selectedOrder?.adminNotes || selectedOrder?.unlockCode || (selectedOrder?.deliveryData && Object.keys(selectedOrder.deliveryData).length > 0)) && (
                <div className="bg-amber-50/50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-xl p-4 space-y-3">
                  <h3 className="font-bold text-amber-800 dark:text-amber-400 flex items-center gap-2 text-sm sm:text-base">
                    <Terminal className="w-5 h-5" /> Retour et données de livraison
                  </h3>

                  {selectedOrder?.unlockCode && (
                    <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-amber-300 text-center space-y-1">
                      <p className="text-xs text-amber-700 dark:text-amber-400 font-semibold uppercase tracking-wider">🔑 Code ou lien de déblocage</p>
                      <p className="text-xl font-mono font-black text-gray-900 dark:text-white select-all bg-gray-50 dark:bg-gray-800 p-2 rounded border border-dashed">
                        {selectedOrder.unlockCode}
                      </p>
                    </div>
                  )}
                  ...
                  {selectedOrder?.deliveryData && Object.keys(selectedOrder.deliveryData).length > 0 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {Object.entries(selectedOrder.deliveryData).map(([key, value]) => (
                        <div key={key} className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-amber-200">
                  ...
                  {selectedOrder?.adminNotes && (
                    <div className="text-sm">
                      <p className="text-xs text-gray-400 font-medium flex items-center gap-1">
                        <MessageSquare className="w-3.5 h-3.5" /> Note de l'administrateur :
                      </p>
                      <p className="mt-1 text-gray-700 dark:text-gray-300 italic bg-white/60 dark:bg-slate-800/60 p-2.5 rounded-lg border">
                        {selectedOrder.adminNotes}
                      </p>
                    </div>
                  )}
            ...
            <div className="p-4 bg-gray-50 dark:bg-gray-700/50 border-t border-gray-200 dark:border-gray-700 flex justify-end">
              <button onClick={closeOrderDetails}
                className="px-5 py-2 bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-gray-800 dark:text-white text-sm font-semibold rounded-xl transition-colors">
                Fermer la vue
              </button>
            </div>
          </div>
...
//           o.status === 'en cours' || o.status === 'En cours'
//         ).length;
