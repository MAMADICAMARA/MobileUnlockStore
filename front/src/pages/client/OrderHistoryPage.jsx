...
const OrderHistoryPage = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        // ✅ FIX : Lire la catégorie depuis l'URL et reset à 'Tous' si absente
        const params = new URLSearchParams(location.search);
        const qcat = params.get('category');
        setFilterCategory(qcat || 'Tous');
  ...
  /**
   * ✅ FIX : Gère le clic sur un onglet de catégorie
   * Met à jour le state ET l'URL pour rester cohérent
   */
  ...
  const filteredOrders = useMemo(() => {
    ...
    // ✅ FIX : La comparaison de catégorie est maintenant fiable
    if (filterCategory !== 'Tous') {
      filtered = filtered.filter(order => {
        const cat = (order?.serviceDetails?.category || order?.service?.category || '')
          .toString()
          .toLowerCase()
          .trim();
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {/* ✅ ONGLETS DE CATÉGORIE — toujours visibles */}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg p-4 border border-gray-100 dark:border-gray-700">
        <div className="flex flex-col gap-4">
          ...
          {/* Filtres par statut (masqués par défaut) */}
          {showFilters && (
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                <button
                  onClick={() => {
                    ...
                    const exportFileDefaultName = `commande-${selectedOrder._id}.json`;
