...
// Essentiel sur mobile : sans ça, l'utilisateur peut scroller la page derrière
// le modal (bug classique iOS Safari / Android Chrome).
...
const OrderModal = ({ order, onClose, onStatusChange }) => {
  ...
  // ✅ Rendu via portail dans <body> : AdminLayout applique un `backdrop-blur`
  // sur le conteneur de page, ce qui crée un "containing block" CSS et piège
  // les enfants en `position: fixed` (ils ne couvrent alors plus l'écran).
  // Le portail contourne totalement ce problème, quel que soit le layout parent.
  return createPortal(
    <div
      // ✅ z-[60] : garanti au-dessus de la sidebar admin (z-50) et de son overlay (z-40)
    ...
    >
      <div className="relative bg-white dark:bg-slate-800 w-full sm:max-w-2xl rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[88vh]">
        ...
        {/* ── Header ── */}
        <div className="relative flex items-start justify-between px-4 sm:px-5 pt-5 sm:pt-5 pb-3 border-b border-slate-100 dark:border-slate-700 flex-shrink-0">
        ...
        <div className="overflow-y-auto flex-1 min-h-0 px-4 sm:px-5 py-4 space-y-4">
          ...
          {/* Statut + montant */}
          <div className="flex items-center justify-between gap-3 p-3 sm:p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
            ...
            <div className="bg-purple-50 dark:bg-purple-900/10 border border-purple-100 dark:border-purple-900/30 rounded-xl overflow-hidden">
              ...
              <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
                {submitted.map(([key, value]) => (
                  <div key={key} className="bg-white dark:bg-slate-700 rounded-xl p-3 border border-purple-100 dark:border-purple-900/30 min-w-0">
          ...
          {order.adminNotes && (
            <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/30 rounded-xl p-4">
              ...
              <p className="text-sm text-slate-700 dark:text-slate-300 break-words">{order.adminNotes}</p>
            </div>
          )}
        ...
        {/* ── Footer ── */}
        {/* pb ajouté : évite que les boutons soient masqués par la barre gestuelle iOS/Android */}
        <div className="px-4 sm:px-5 py-3 sm:py-4 pb-[calc(0.75rem+env(safe-area-inset-bottom))] sm:pb-4 border-t border-slate-100 dark:border-slate-700 flex gap-2 sm:gap-3 flex-shrink-0">
          {!isRembourse && (
            <button
              onClick={() => {
...
const AdminOrdersPage = () => {
  ...
  return (
    <div className="w-full min-w-0 py-4 sm:py-6 px-3 sm:px-6 overflow-x-hidden">
      <div className="max-w-7xl mx-auto space-y-4 sm:space-y-5">

        {/* En-tête */}
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-xl sm:text-3xl font-bold text-slate-900 dark:text-white truncate">Commandes</h1>
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              {filteredOrders.length} commande{filteredOrders.length !== 1 ? 's' : ''}
            </p>
          </div>
          <button onClick={() => fetchOrders(true)} disabled={refreshing}
            className="flex-shrink-0 flex items-center gap-2 px-3 sm:px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-600 dark:text-slate-300 text-sm font-medium hover:bg-slate-50 transition-colors shadow-sm disabled:opacity-60">
      ...
      {/* ── Pagination ── */}
      {pagination.pages > 1 && (
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 py-3 mt-4 sm:mt-5 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
