...
const ServiceForm = ({ service, onSubmit, isLoading, onCancel }) => {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm({
    defaultValues: service ? {
      ...
      // ✅ Lire depuis le champ direct du modèle EN PRIORITÉ, sinon depuis metadata
      minQuantity: service.minQuantity ?? service.metadata?.credit?.minQuantity ?? 10,
      maxQuantity: service.maxQuantity ?? service.metadata?.credit?.maxQuantity ?? 100,
    } : {
      ...
      minQuantity: 10,   // ✅ Défaut à 10 pour les nouveaux services Credit
      maxQuantity: 100,
    }
  });
  ...
  const handleImageUpload = (e) => {
    ...
    reader.onloadend = () => { setImagePreview(reader.result); setValue('imageUrl', reader.result); };
  ...
  const addCustomField    = () => setCustomFields([...customFields, { name: `custom_${Date.now()}`, label: '', type: 'text', required: true, placeholder: '' }]);
  ...
  const onFormSubmit = (data) => {
    ...
    if (backendCategory === 'Credit') {
      const minQty = Math.max(Number(data.minQuantity) || 10, 1);
      const maxQty = Math.max(Number(data.maxQuantity) || 100, minQty);
      ...
      fieldsRequired.push({ name: 'quantity', label: 'Quantité',           type: 'number', required: true, defaultValue: minQty, min: minQty, max: maxQty, helpText: `Minimum ${minQty} crédit(s), maximum ${maxQty} crédit(s)` });
    }
    ...
    const metadata = {
      ...
      credit:  { minQuantity: minQty, maxQuantity: maxQty },
    };
    ...
    onSubmit({
      ...
      fieldsRequired,
      metadata,
      // ✅ NOUVEAU : envoyer aussi les champs directs du modèle Service
      minQuantity: backendCategory === 'Credit' ? minQty : 1,
      maxQuantity: backendCategory === 'Credit' ? maxQty : 0,
    });
  };
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-5">
      ...
      <div className="bg-gray-50/60 dark:bg-slate-900/30 border border-gray-100 dark:border-slate-700 rounded-2xl p-4 sm:p-5">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" {...register('name', { required: 'Le nom est obligatoire' })}
              placeholder="Ex: Déblocage IMEI, Crédit TFM Tool Pro..." className={inputClass} />
          ...
          <div>
            ...
            <textarea {...register('description', { required: 'La description est obligatoire' })} rows={3}
          ...
          <label className="flex items-center gap-2.5 p-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-xl cursor-pointer">
            <input type="checkbox" {...register('active')} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 flex-shrink-0" />
      ...
      <div className="bg-gray-50/60 dark:bg-slate-900/30 border border-gray-100 dark:border-slate-700 rounded-2xl p-4 sm:p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select {...register('category', { required: 'La catégorie est obligatoire' })} className={`${inputClass} cursor-pointer`}>
          ...
          <div className="sm:col-span-2">
            ...
            <input type="text" {...register('deliveryTime', { required: 'Le délai est obligatoire' })}
              placeholder="Ex: 24h, 2-3 jours, Instantané" className={inputClass} />
      ...
      <div className="bg-gray-50/60 dark:bg-slate-900/30 border border-gray-100 dark:border-slate-700 rounded-2xl p-4 sm:p-5">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1.5">Télécharger une image</label>
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                <input id="imageUploadInput" type="file" accept="image/*" onChange={handleImageUpload}
              ...
              <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">Formats acceptés : PNG, JPG, WEBP (max 2 Mo)</p>
            </div>
            <div>
              {imagePreview ? (
      ...
      <div className="bg-gray-50/60 dark:bg-slate-900/30 border border-gray-100 dark:border-slate-700 rounded-2xl p-4 sm:p-5">
        ...
        <textarea {...register('instructions')} rows={6}
      ...
      {selectedCategory && categoryFields[selectedCategory]?.length > 0 && (
        <div className="bg-gray-50/60 dark:bg-slate-900/30 border border-gray-100 dark:border-slate-700 rounded-2xl p-4 sm:p-5">
          <SectionHeader icon={ListChecks} title={`Champs requis — ${categoryMapping[selectedCategory]?.display || selectedCategory}`} />

          {/* Info spéciale pour Credit */}
          {selectedCategory === 'Credit' && (
            <div className="mb-3 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl">
              <p className="text-xs text-green-700 dark:text-green-300 flex items-start gap-1.5">
                ...
                Ces valeurs seront stockées dans la base de données et utilisées pour bloquer les commandes inférieures au minimum.
              </p>
            </div>
          )}
              ...
              <div key={field.name} className="flex items-start gap-3">
                ...
                {field.type === 'number' && (
                  <div className="flex-1">
                    ...
                    <input type="number"
                      {...register(field.name, { min: field.min || 1, max: field.max || 10000, valueAsNumber: true })}
      ...
      <div className="bg-gray-50/60 dark:bg-slate-900/30 border border-gray-100 dark:border-slate-700 rounded-2xl p-4 sm:p-5">
        <div className="flex justify-between items-center mb-3">
          ...
          <button type="button" onClick={addCustomField}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors flex-shrink-0">
