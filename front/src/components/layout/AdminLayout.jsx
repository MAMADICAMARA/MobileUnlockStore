...
// Persiste le choix dans localStorage, respecte la préférence système par défaut,
// et applique/enlève la classe "dark" sur <html> (nécessite darkMode: 'class' dans tailwind.config.js)
const useTheme = () => {
...
// ─── Bouton toggle dark/sun ────────────────────────────────────────────────────
const ThemeToggle = ({ theme, onToggle }) => (
  <button
    onClick={onToggle}
    title={theme === 'dark' ? 'Passer en mode clair' : 'Passer en mode sombre'}
    className="relative w-10 h-10 flex items-center justify-center rounded-lg bg-white/5 border border-white/10 text-gray-300 hover:text-white hover:bg-white/10 transition-colors"
  >
    <Sun className={`w-5 h-5 absolute transition-all duration-300 ${theme === 'dark' ? 'opacity-0 -rotate-90 scale-50' : 'opacity-100 rotate-0 scale-100'}`} />
    <Moon className={`w-5 h-5 absolute transition-all duration-300 ${theme === 'dark' ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 rotate-90 scale-50'}`} />
  </button>
);
...
const AdminLayout = () => {
  ...
  const navItems = [
    ...
    { to: "/admin/services/IMEI", icon: Smartphone, label: "Catégorie IMEI", color: "from-blue-400 to-cyan-400" },
    { to: "/admin/services/Server", icon: Server, label: "Catégorie Serveur", color: "from-orange-400 to-amber-400" },
    { to: "/admin/services/Rental", icon: Globe, label: "Catégorie Location", color: "from-purple-400 to-pink-400" },
    { to: "/admin/services/License", icon: Key, label: "Catégorie Licence", color: "from-green-400 to-emerald-400" },
    { to: "/admin/orders", icon: ShoppingBag, label: "Commandes", color: "from-pink-400 to-rose-400" },
    { to: "/admin/orders?category=IMEI", icon: ShoppingBag, label: "IMEI", color: "from-pink-400 to-rose-400" },
    { to: "/admin/orders?category=Server", icon: ShoppingBag, label: "Serveur", color: "from-pink-400 to-rose-400" },
    { to: "/admin/orders?category=Rental", icon: ShoppingBag, label: "Location", color: "from-pink-400 to-rose-400" },
    { to: "/admin/orders?category=License", icon: ShoppingBag, label: "Licence", color: "from-pink-400 to-rose-400" },
  ...
  return (
    // ⚠️ FIX: min-w-0 sur le conteneur flex racine pour éviter que le contenu
    // (tableaux larges, etc.) ne force un débordement horizontal de toute la page.
    <div className="flex min-h-screen w-full min-w-0 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      {/* SIDEBAR */}
      <aside
        className={`
        ...
        `}
      >
        {/* Éléments décoratifs (contenus, ne débordent jamais grâce à overflow-hidden) */}
        ...
        <div className="relative z-10">
          ...
          {/* Logo Desktop — ✅ FIX: bouton "dark mode" égaré retiré d'ici (il vit maintenant dans le header, à côté de la cloche) */}
          <div className="hidden md:flex items-center gap-3 mb-8 p-3 rounded-xl bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-white/5">
            ...
            <div className="min-w-0">
              <h2 className="text-xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 text-transparent bg-clip-text truncate">
                Espace Admin
              </h2>
              <p className="text-xs text-gray-400 truncate">{user?.email || 'Administrateur'}</p>
            </div>
          </div>
        </div>
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <item.icon className={`w-5 h-5 flex-shrink-0 transition-all duration-300 ${isActive ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
        ...
        <div className="relative z-10 mt-6 space-y-3">
          <div className="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500/20 to-pink-500/20 border border-amber-500/20">
            <p className="text-xs text-amber-400 flex items-center gap-2">
              ...
              <span className="truncate">Connecté en tant qu'administrateur</span>
            </p>
          </div>
      ...
      {/* Contenu principal — ⚠️ FIX: min-w-0 pour permettre au flex-item de rétrécir
          en dessous de la largeur intrinsèque de son contenu (tableaux, etc.) */}
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <header className="sticky top-0 z-30 bg-slate-900/80 dark:bg-slate-950/80 backdrop-blur-xl border-b border-white/10 shadow-lg">
          <div className="px-4 md:px-8 py-4 flex items-center justify-between gap-3">
            {/* Bouton menu mobile */}
            <button
              onClick={toggleSidebar}
              className="md:hidden text-white p-2 rounded-lg hover:bg-white/10 transition-colors relative group flex-shrink-0"
            >
              <Menu size={24} />
            ...
            <h1 className="text-lg md:text-xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 text-transparent bg-clip-text truncate min-w-0 flex-1 md:flex-none">
              {navItems.find(item => item.to === location.pathname)?.label || 'Administration'}
            </h1>

            {/* Actions header */}
            <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
              {/* ✅ Toggle dark/sun, juste à côté de la cloche */}
        ...
        <main className="flex-1 min-w-0 p-4 md:p-8 overflow-auto">
          <div className="relative min-w-0">
            {/* Éléments décoratifs — ✅ FIX: overflow-hidden ajouté pour empêcher
                les cercles flous positionnés en absolute de créer un débordement
                horizontal (c'était une cause probable du souci "pas responsive") */}
            ...
            <div className="relative mx-auto w-full max-w-6xl min-w-0">
              <div className="relative z-10 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-4 sm:p-6 shadow-xl overflow-x-hidden">
                <Outlet />
              </div>
            </div>
          </div>
...
//     { to: "/admin/recharge", icon: RefreshCw, label: "Recharger un client", color: "from-green-400 to-emerald-400" },
//     { to: "/admin/services", icon: Server, label: "Services", color: "from-yellow-400 to-orange-400" },
//     // liens vers catégories de services
//     { to: "/admin/services/IMEI", icon: Smartphone, label: "Catégorie IMEI", color: "from-blue-400 to-cyan-400" },
//     { to: "/admin/services/Server", icon: Server, label: "Catégorie Serveur", color: "from-orange-400 to-amber-400" },
//     { to: "/admin/services/Rental", icon: Globe, label: "Catégorie Location", color: "from-purple-400 to-pink-400" },
//     { to: "/admin/services/License", icon: Key, label: "Catégorie Licence", color: "from-green-400 to-emerald-400" },
