...
const ServiceModal = ({ isOpen, onClose, service, userBalance }) => {
  ...
  useEffect(() => {
    ...
    setUploadProgress(0);
    setQuantity(MIN_QUANTITY); // reset quantité à chaque ouverture
  ...
  // ── Calcul du prix ───────────────────────────────────────────────────────
  // Pour les licences : prix unitaire (par crédit) × quantité
  // Pour les autres   : prix fixe du service
  const unitPrice  = service.price || 0;
  const totalPrice = isLicense ? unitPrice * quantity : unitPrice;
  ...
  const getFormFields = (svc) => {
    ...
    const defaults = {
      ...
      // Pour License : l'email suffit, la quantité est gérée séparément
      License: [
  ...
  const validateForm = () => {
    // Validation quantité pour les licences
    if (isLicense && quantity < MIN_QUANTITY) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Pour les licences, on envoie la quantité et le prix total calculé
      const orderPayload = {
        serviceId: service._id,
        userSubmittedData: isLicense
          ? { ...formFields, quantity: quantity.toString() }
          : formFields,
        ...(isLicense && { quantity, totalPrice }),
      };
        ...
        <div className="overflow-y-auto flex-1 p-6 pt-7">
          ...
          {step === 1 && !fetchingDetails && (
            <div className="space-y-4">
              {/* Sélecteur de quantité en premier pour les licences */}
              {isLicense && renderQuantitySelector()}
                  ...
                  {isLicense ? (
                    <>
                      ...
                      <div className="flex justify-between">
                        ...
                        <dd className="font-bold text-gray-900 dark:text-white">{quantity}</dd>
                      </div>
                      <div className="flex justify-between pt-1 border-t border-dashed border-gray-300 dark:border-gray-600">
                        <dt className="text-gray-600 dark:text-gray-300 font-medium">Sous-total ({quantity} × {new Intl.NumberFormat('fr-FR').format(unitPrice)} FG)</dt>
...
//                 <p className={`text-2xl font-bold ${theme.text}`}>{service.price?.toFixed(2)} FG</p>
//               </div>
//             </div>
//             {service.deliveryTime && (
//               <div className="flex items-center gap-1.5 mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 text-sm text-gray-500 dark:text-gray-400">
...
//               </div>
//               <span className={`font-bold ${isBalanceSufficient ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
//                 {currentBalance.toFixed(2)} FG
//               </span>
//             </div>
//             {isBalanceSufficient && (
//               <p className="mt-1.5 text-sm text-gray-500 dark:text-gray-400">
//                 Après commande : <span className="font-semibold text-green-600 dark:text-green-400">{remainingBalance.toFixed(2)} FG</span>
//               </p>
//             )}
//           </div>
...
//                   <div className="flex justify-between">
//                     <dt className="text-gray-500 dark:text-gray-400">Prix</dt>
//                     <dd className="font-medium text-green-600 dark:text-green-400">{service.price?.toFixed(2)} FG</dd>
//                   </div>
//                   {formFieldDefs.map(field => {
//                     const value = formFields[field.name];
//                     if (!value) return null;
...
//                   <div className="flex justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
//                     <dt className="font-medium text-gray-900 dark:text-white">Solde après commande</dt>
//                     <dd className="font-bold text-green-600 dark:text-green-400">{remainingBalance.toFixed(2)} FG</dd>
//                   </div>
//                 </dl>
//               </div>
