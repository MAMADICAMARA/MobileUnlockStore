...
// ─── Constante : délai d'inactivité (15 minutes) ─────────────────────────────
...
const AuthProvider = ({ children }) => {
  ...
  const logout = useCallback(() => {
    ...
    localStorage.removeItem(LAST_ACTIVITY_KEY); // ✅ Nettoyer le timestamp
    setToken("");
    setUser(null);
  ...
  // ✅ Enregistre le timestamp de la dernière activité dans localStorage
  const stampActivity = useCallback(() => {
    localStorage.setItem(LAST_ACTIVITY_KEY, String(Date.now()));
  }, []);

  const resetInactivityTimer = useCallback(() => {
    stampActivity(); // ✅ Persiste l'activité même si la page est réduite ensuite
    if (inactivityTimer.current) clearTimeout(inactivityTimer.current);
    inactivityTimer.current = setTimeout(logout, INACTIVITY_DELAY);
  }, [logout, stampActivity]);

  // ✅ Vérifie si l'inactivité dépasse 15 min en lisant le timestamp stocké
  const checkInactivityFromTimestamp = useCallback(() => {
    const lastActivity = parseInt(localStorage.getItem(LAST_ACTIVITY_KEY) || '0', 10);
    if (!lastActivity) return; // Pas encore d'activité enregistrée
    const elapsed = Date.now() - lastActivity;
    if (elapsed >= INACTIVITY_DELAY) {
      console.warn(`[AuthContext] Inactivité détectée : ${Math.round(elapsed / 60000)} min → déconnexion`);
      logout();
    }
  }, [logout]);

  // ✅ Écoute les événements DOM pour l'activité
  useEffect(() => {
    if (!user) return;

    const events = ['mousemove', 'keydown', 'click', 'scroll', 'touchstart'];
    events.forEach(e => window.addEventListener(e, resetInactivityTimer, { passive: true }));
    resetInactivityTimer(); // Démarrer le timer au montage

    // ✅ CLEF : vérifier l'inactivité quand l'utilisateur revient sur la page
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        checkInactivityFromTimestamp();
      }
    };
  ...
  }, [user, resetInactivityTimer, checkInactivityFromTimestamp]);
  ...
  const login = useCallback((userData) => {
    ...
    stampActivity(); // ✅ Enregistrer l'activité au login
    setToken(raw);
    setUser(userObject);
  }, [stampActivity]);
  ...
  // ✅ Initialisation : vérifier token + inactivité au chargement de la page
  useEffect(() => {
    const raw = readStoredToken();
    ...
    // ✅ Vérifier l'inactivité dès le chargement de la page
    // (cas : l'utilisateur revient après avoir fermé/rouvert le navigateur)
    const lastActivity = parseInt(localStorage.getItem(LAST_ACTIVITY_KEY) || '0', 10);
    if (lastActivity && Date.now() - lastActivity >= INACTIVITY_DELAY) {
      console.warn("[AuthContext] Inactivité détectée au chargement → déconnexion");
      logout();
      setLoading(false);
      return;
    }

    // Construire le user depuis localStorage ou JWT
    ...
    // Fetcher le profil depuis l'API pour avoir le vrai solde
    fetch(`${API_BASE}/api/auth/profile`, {
      headers: { Authorization: `Bearer ${raw}`, 'Content-Type': 'application/json' },
    })
      .then(r => r.ok ? r.json() : null)
