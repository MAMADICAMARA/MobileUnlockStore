...
exports.createService = async (req, res) => {
  try {
    ...
    if (!name || !description || price === undefined || !category) {
      return res.status(400).json({ success: false, message: 'Les champs name, description, price et category sont requis' });
    }
    const validCategories = ['IMEI', 'Server', 'Rental', 'Credit'];
    if (!validCategories.includes(category)) {
      return res.status(400).json({ success: false, message: `La catégorie doit être l'une des suivantes: ${validCategories.join(', ')}` });
    }
    const existingService = await Service.findOne({ name });
    if (existingService) {
      return res.status(400).json({ success: false, message: 'Un service avec ce nom existe déjà' });
    }
    const service = await Service.create({ name, description, price: parseFloat(price), category, active: true });
    res.status(201).json({ success: true, message: 'Service créé avec succès', data: service });
  } catch (error) {
...
exports.updateService = async (req, res) => {
  try {
    ...
    await service.save();
    res.status(200).json({ success: true, message: 'Service mis à jour avec succès', data: service });
  } catch (error) {
...
exports.deleteService = async (req, res) => {
  try {
    ...
    await service.save();
    res.status(200).json({ success: true, message: 'Service désactivé avec succès' });
  } catch (error) {
...
/**
 * ✅ CORRIGÉ : limit par défaut à 1000 pour récupérer tous les services
 */
exports.getServices = async (req, res) => {
  try {
    const {
      ...
      active = true,
      page  = 1,
      limit = 1000,   // ✅ Tous les services par défaut
      sort  = '-createdAt',
    } = req.query;
    ...
    res.status(500).json({ success: false, message: 'Erreur serveur lors de la récupération des services' });
  }
};
...
exports.getUserFieldsForCategory = async (req, res) => {
  try {
    ...
    res.json({ success: true, category, userFields, deliveryFields });
  } catch (error) {
